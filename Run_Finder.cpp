...
Run_Finder::Run_Finder (Nominal_Frequency_kHz nom_freq, Tag_Set *tags, string prefix) :
  nom_freq(nom_freq),
  tags(tags),
  graph_by_lotek_id(),
  cands(),
...
Run_Finder::setup_graphs() {
  ...
  // For depth up to Run_Candidate::hits_to_confirma_id, add appropriate nodes to the graph.
  ...
  for (unsigned int depth = 1; depth <= Run_Candidate::hits_to_confirm_id + 1; ++depth) {
    // loop over all Tag_ID_Sets at this level
    for (DFA_Graph::Node_For_IDs::iterator it = graph.get_node_for_set_at_depth()[depth - 1].begin(); 
	 it != graph.get_node_for_set_at_depth()[depth - 1].end(); ++it) {
	  
      // a map of gap sizes to compatible tag IDs
	  
      interval_map < Gap, Tag_ID_Set > m;
	  
      // sanity check: make sure there's only one tag ID left after hits_to_confirma pair of bursts
...
void
Run_Finder::set_default_pulse_slop_ms(float pulse_slop_ms) {
...
void
Run_Finder::set_default_hit_rate_window(Gap h) {
...
float
Run_Finder::get_tag_hit_rate(Tag_ID tid) {
...
float *
Run_Finder::get_true_gaps(Tag_ID tid) {
...
void
Run_Finder::dump_bogus_burst(Pulse &p, Tag_ID tag_id) {
  Tag_Candidate::dump_bogus_burst(p.ts, tag_id, p.ant_freq, -96, -96, out_stream);
};

Gap Run_Finder::default_pulse_slop = 0.0015; // 1.5 ms
...
Gap Run_Finder::default_hit_rate_window = 60;
